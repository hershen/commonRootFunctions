...
TOFtiming::TOFtiming(const std::string pathToFiles, const int runNum)
    : m_eventNumber(std::make_shared<Long64_t>(-1)), m_ch4Time(std::make_shared<double>(0.0)),
      m_ch6Time(std::make_shared<double>(0.0)), m_ch12Time(std::make_shared<double>(0.0)),
      m_ch13Time(std::make_shared<double>(0.0)), m_ch4Error(std::make_shared<double>(0.0)),
      m_ch6Error(std::make_shared<double>(0.0)), m_ch12Error(std::make_shared<double>(0.0)),
      m_ch13Error(std::make_shared<double>(0.0)), m_electronSimpleTOFmean(0.0), m_electronSimpleTOFsigma(0.0),
      m_muonSimpleTOFmean(0.0), m_muonSimpleTOFsigma(0.0), m_pionSimpleTOFmean(0.0), m_pionSimpleTOFsigma(0.0),
      m_timeErrorScaling(50.0), m_calculatedSimpleTOFmean_sigma(false), m_runNum(runNum), m_treeName("timingAndWaveforms"),
...
TFitResultPtr TOFtiming::fitTOF(const bool boundBeta) const {
  ...
  TGraphErrors graph(3, (std::array<double, 3>{getX0(), getX1(), getX2()}).data(),
                     ...
                     (std::array<double, 3>{getT0Error(), getT1Error(), getT2Error()}).data());

  // Fit function
  TF1 function("function", m_TOFfunctionString.data(), -4, 1); // 0 - intercept, 1 - speed

  std::string fitOptions =
      "E M EX0 S Q"; // E - better errors, M - Minos, EX0 - Don't use errors on x values, S - return smaprt ptr, Q - queit
  if (boundBeta) {
...
double TOFtiming::getX0() const {
  return -myFuncs::testbeam::c_downstreamTOF2upstreamTOFdistance -
         RunDB::instance()[getRunNum()].getDownstream2crystalCenterDistance();
}
...
double TOFtiming::getX2() const { return -RunDB::instance()[getRunNum()].getDownstream2crystalCenterDistance(); }

bool TOFtiming::isSimpleElectron(const double numSigmas) {
  // Make sure mean and sigma have been calculated
  if (!m_calculatedSimpleTOFmean_sigma)
    calculateSimpleTOFmean_sigma();
...
void TOFtiming::calculateSimpleTOFmean_sigma() {
  ...
  for (Long64_t iEntry = 0; iEntry < entries; ++iEntry)
    simpleTOFhist.Fill(getSimpleTOF(iEntry));
