...
void MVectorTemplate::resetTemplateRange() {
	if(m_tF1.IsValid()) m_tF1.SetRange(getXvalueOfFirstTemplateEntry(), getXvalueOfFirstTemplateEntry() + m_dx * (m_templateValues.size()-1) );	
}
...
bool MVectorTemplate::goodFit(const TFitResultPtr& fitResult) const{
	...
	if( fitStatus ) {
		if(fitStatus != 4000) {
			if (m_debugLevel > 0) std::cerr << "MVectorTemplate::goodFit : fit failed, fit status = " << fitStatus << ". Not adding vector!" << std::endl;
			return false;
		}
		else if(m_debugLevel > 0) std::cout << "MVectorTemplate::goodFit : MINOS falied, fit status = " << fitStatus << ". Vector MIGHT still be added!!!" << std::endl;
	}
	...
	{
		if(m_debugLevel > 0) std::cerr << "MVectorTemplate::goodFit : fit failed, Chi2/NDF of fit = " << chi2_ndf << " is out of limits. limits are (" << m_minChi2_NdfLimit << "," << m_maxChi2_NdfLimit << "). Not adding vector!" << std::endl;
		return false; 
	}
	
	if(m_debugLevel > 10) std::cout << "MVectorTemplate::goodFit : fittedAmplitude = " << fittedAmplitude << ", fittedPedestal = " << fittedPedestal << ", fittedXshift = " << fittedXshift << ", chi2 / NDF = " << chi2_ndf << std::endl;
	...
	{
		if(m_debugLevel > 10) std::cerr << "MVectorTemplate::goodFit : fit failed, reached amplitude limit. fittedAmplitude = " << fittedAmplitude << ". limits are (" << m_minAmplitudeLimit << "," << m_maxAmplitudeLimit << "). Not adding vector!" << std::endl;
		return false;
	}
	...
	{
		if(m_debugLevel > 10) std::cerr << "MVectorTemplate::goodFit : fit failed, reached xShift limit. fittedXshift = " << fittedXshift << ". limits are (" << m_minXshiftLimit << "," << m_maxXshiftLimit << "). Not adding vector!" << std::endl;
		return false;
	}
	...
	{
		if(m_debugLevel > 10) std::cerr << "MVectorTemplate::goodFit : fit failed, reached pedestal limit. fittedPedestal = " << fittedPedestal << ". limits are (" << m_minPedestalLimit << "," << m_maxPedestalLimit << "). Not adding vector!" << std::endl;
		return false;
	}
...
TFitResult MVectorTemplate::fitFunctionToVector(const std::vector<double>& vector, const double std, TF1& function) {
	...
	if(m_debugLevel > 15)
		std::cout << "MVectorTemplate::fitFunctionToVector : amplitudeGuess = " << amplitudeGuess << ", pedestalGuess = " << pedestalGuess << ", xShiftGuess = " << xShiftGuess << std::endl;
	...
	//We then Change the function range so it takes into account the xShift. Otherwise, the ends of the function are not correctly represented and the averaging is not good.
	double minRange, maxRange;
	function.GetRange(minRange,maxRange);
	if(getDebugLevel() > 15)
		std::cout << "MVectorTemplate::fitFunctionToVector : About to fit in the range = (" << minRange + fitResult->Parameter(2) << "," << maxRange + fitResult->Parameter(2) << ")" << std::endl;
...
void MVectorTemplate::clipTemplateEnds(const double xOfNewVector_0, const size_t newVectorSize) {
	...
	if(getDebugLevel() > 20)
		std::cout << "MVectorTemplate::clipTemplateEnds : timeOfLastTemplateEntry = " << timeOfLastTemplateEntry << ", timeOfLastNewVectorValue = " << timeOfLastNewVectorValue << std::endl;
	...
	if(getDebugLevel() > 10)
		std::cout << "MVectorTemplate::clipTemplateEnds : firstTemplateIdx = " << firstTemplateIdx << ", lastTemplateIdx = " << lastTemplateIdx << std::endl;
...
// 		gPad->WaitPrimitive();
		...
		//fittedXshift rescales the time of newVector. newVector[0] corresponds to time xOfNewVector_0, newVector[1] corresponds to time (xOfNewVector_0 + m_dx)...
		
		//Clip first and end of template, if they don't overlap in time with the newVector. We want to keep only the parts that can be averaged.
		clipTemplateEnds(xOfNewVector_0, newVector.size());
		
		assert(getXvalueOfFirstTemplateEntry() >= xOfNewVector_0);
		
		//newAlignedVector has it's values time aligned to template values, time of 0'th element of newTimeAlignedVector has same time as templateVlaues[0]
    ...
    else
    {
      ...
      firstTemplateIdx = 2, lastTemplateIdx = m_templateValues.size()-1
		...
		if(getDebugLevel() > 10)
			std::cout << "MVectorTemplate::addVector : About to average in the range (" << getXvalueOfFirstTemplateEntry() << "," << getXvalueOfFirstTemplateEntry() + (getTemplateSize()-1)*m_dx << ")" << std::endl;
    ...
    for(size_t iTemplateIdx = 0; iTemplateIdx < getTemplateSize(); ++iTemplateIdx)
    {
			...
			if(getAverageMode() == noWeight)  {
				...
				m_templateValues[iTemplateIdx] = (templateValue * static_cast<double>(m_numAveragedFuncs) + newVectorValue) * ov_numAveragedFuncsP1;
			}

			if(getDebugLevel() > 30) {
				std::cout << "MVectorTemplate::addVector : Averaging template iTemplateIdx " << iTemplateIdx << ", time = " << getXvalueOfFirstTemplateEntry() + iTemplateIdx*m_dx << ". newVectorValue = " << newVectorValue << ", m_templateValues[iTemplateIdx] after averaging = " << m_templateValues[iTemplateIdx] << std::endl;
			}
