...
eclCrystalDB::eclCrystalDB(std::string fullFileName)
  : maxThetaId(-1)
{
  int cellIdPointer = 0;
  ...
  std::vector<std::string> branchNames = {cellIdBranchName, thetaIdBranchName, phiIdBranchName, positionXbranchName, positionYbranchName, positionZbranchName, directionXbranchName, directionYbranchName, directionZbranchName};
  
  std::vector<void *> pointers = {&cellIdPointer, &thetaIdPointer,&phiIdPointer, &positionXpointer, &positionYpointer, &positionZpointer, &directionXpointer, &directionYpointer, &directionZpointer};
  ...
  for(auto entry = 0; entry < inputChain->GetEntries(); ++entry)
  {
    ...
    crystalInfo tempCrystal{cellIdPointer, thetaIdPointer, phiIdPointer, positionXpointer, positionYpointer, positionZpointer, directionXpointer, directionYpointer, directionZpointer};
...
eclCrystalDB::~eclCrystalDB(void)
{
...
//     
double eclCrystalDB::getPhi(const int &cellID) const
{
  return getPosition(cellID).Phi();  
}

double eclCrystalDB::getTheta(const int &cellID) const
{
  return getPosition(cellID).Theta();  
}
...
int eclCrystalDB::getThetaID(const int &cellID) const
{
  return crystalsDB[cellID].thetaID;  
}

int eclCrystalDB::getPhiID(const int &cellID) const
{
  return crystalsDB[cellID].phiID;  
}
...
int eclCrystalDB::getMaxThetaId() const
{
  ...
  if(maxThetaId >= 0 ) return maxThetaId;
  
  //Else, look for it
  for(auto crystal : crystalsDB)
  {
    if(crystal.thetaID > maxThetaId) maxThetaId = crystal.thetaID;
  }
...
int eclCrystalDB::getRegion(const int &cellId) const
{
  ...
  if(theta < FEthetaMax) return 1; //forward endcap
  if(theta > BEthetaMin) return 3; //backward endcap
  else return 2; //barrel
}
