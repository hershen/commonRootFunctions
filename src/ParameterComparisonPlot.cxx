...
ParameterComparisonPlot::ParameterComparisonPlot() :
	...
	m_drawRectangleForLastEntry(true)
	{}
...
void ParameterComparisonPlot::drawRectangleForLastEntry() {
	
	//Get last point's information
	double xParValue,y;
	this->GetPoint(this->GetN() - 1, xParValue, y);
	const double xParError = this->GetErrorX(this->GetN() - 1);
	...
	m_lastEntryRectangle = new TPaveText( xParValue - xParError, gPad->GetUymin() + xTickLength, xParValue + xParError, gPad->GetUymax() - xTickLength,"NB" );
	
	m_lastEntryRectangle->Draw();
}
...
void ParameterComparisonPlot::Draw(const std::string& options) {
	...
	//Draw this rectangle first so that the graph's points are drawn on top of it
	if(getDrawRectangleForLastEntry())
		drawRectangleForLastEntry();
