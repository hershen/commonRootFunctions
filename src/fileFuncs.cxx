...
{       
  ...
  TChain * openChain(const std::string& treeName, const std::string& fileNamesString)
  {
    ...
    {
      std::cout << "fileFuncs::openChain: Error - TTree with name '" << treeName.data() << "' in files with string '" << fileNamesString.data() << "' has no branch (it's probably empty). Returning 0." << std::endl;
//       delete branchArray;
      delete chain;
      return 0;
  ...
  int setChainBranch(TChain * chain, const std::vector<std::string>& branchNamesV, std::vector<void*> pointerV)
  {
    ...
    {
      std::cout << "fileFuncs::setChainBranch: Error - branchNamesV.size() = " << branchNamesV.size() << " != pointerV.size() = " << pointerV.size() << ". Aborting" << std::endl;
      return 1001;
    }
    ...
    if( returnVal )
    {
      std::cout << "fileFuncs::setChainBranch: Error - Error while setting branch address - check branch names. Aborting" << std::endl;
      return returnVal;
    }
  ...
  TChain * openChain_setBranch(const std::string& fileNameString, const std::string& treeName, const std::vector<std::string>& branchNamesV, std::vector<void*> pointerV)
  {
    ...
    if( returnVal)
    {
      std::cout << "fileFuncs::openChain_setBranch: Could not set branch addresses. Recieved returnVal = " << returnVal << ". Aborting" << std::endl;
      delete chain;
      return 0;
    }
...
//     #include "dirent.h"
	...
	if( myFuncs::endsWith(fileNameString, ending ) ) rootFilesVec.push_back( dirString + "/" + fileNameString );
      }
      closedir (dir);
    } 
    else 
