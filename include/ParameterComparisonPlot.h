...
// Root fails to properly use the assignment operator for TPaveText (i.e. TPaveText pt; pt = TPaveText(x,y,x2,y2);
// pt->AddText("AA"); - there's a break segmentation).  Therefore, either set x1, x2, y1, y2, options of TPaveText manually before
// using it (might not set TAttText options).  Or try to see if the missing
// TAttText(22,0,gStyle->GetTextColor(),gStyle->GetTextFont(),0) from the constructor is the thing that causes the break
// segmentation
namespace myFuncs {
