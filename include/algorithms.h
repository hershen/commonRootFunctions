...
namespace myFuncs {
	...
	FwItir findFirstBigger(FwItir first, FwItir last, T threshold) {
		return std::find_if(first, last, [threshold](typename std::iterator_traits<FwItir>::value_type value){return value > threshold;});//first element greater than threshold
	}
...
// 		std::cout << "rbegin = "<< *rbegin << ", rend = " << *rend << std::endl;
		const auto reverseLastSmaller = std::find_if(rbegin, rend, [threshold](typename std::iterator_traits<FwItir>::value_type value){return value < threshold;} );
		
		//If nothing found return last (this is because we're going to do return ( reverseLastSmaller + 1).base(); and base moves one after the reverse iterator)
